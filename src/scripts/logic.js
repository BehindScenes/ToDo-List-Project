...
document.getElementById('addTask').addEventListener('click', () => {
    ...
    if (!taskName || !taskDate || !taskColor || !Category)
        return showAlert('Somthing went wrong, please check your form and try again.', 'alert-warning', 'bi bi-exclamation-circle');
    ...
    showAlert('New task has been add successfully.', 'alert-success', 'bi bi-file-earmark-plus');
    autoDeleteAlert();
});
...
function renderTasks(filter = null) {
    ...
    selected.forEach((task, index) => {
        ...
        tasksContainer.innerHTML += `
            <div class="task-item border border-${task.taskColor} rounded px-3 py-2 mb-1">
                <div class="d-flex justify-content-between align-items-start">
                    <div class="fw-semibold text-${task.taskColor} fs-6" style="${task.chk ? "text-decoration: line-through;" : ""}">
                        📌 ${task.taskName} <span class="badge text-bg-${task.taskColor}">${task.taskCategory}</span>
                    </div>
                ...
                <div class="d-flex justify-content-between align-items-center mt-2">
                    <small class="text-${task.taskColor}">📅 ${task.taskDate}</small>
                    <span class="vr text-${task.taskColor}"></span>

                    <small class="text-${task.taskColor}"><i class="bi bi-caret-right-fill"></i>Status:  ${task.chk ? 'Completed' : 'Panding / In progress'}</small>
                    <span class="vr text-${task.taskColor}"></span>
                    <div class="form-check">
                        <label class="form-check-label text-${task.taskColor} small">Completed</label>
                        <input class="form-check-input border-${task.taskColor}" type="checkbox" ${task.chk ? 'checked' : ''} title="${!task.chk ? 'Completed' : 'Not Completed'}" id="taskCheckbox" onchange="toggleCompleted(${index}, this)">
                    </div>
                </div>
            </div>
        `;
...
function deleteTask(index) {
    ...
    showAlert('Your task has been deleted successfully.', 'alert-danger', 'bi bi-trash');
    autoDeleteAlert();
}
...
document.getElementById('colorSelect').addEventListener('change', () => {
    ...
    if (colorSelect)
       return circleColor.classList.add(`text-${colorSelect}`);
});
